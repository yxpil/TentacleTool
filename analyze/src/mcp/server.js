...
class McpStreamableHttpServer {
  constructor(options = {}) {
    ...
    this.serverName = options.serverName || 'analyze-mcp';
    this.serverVersion = options.serverVersion || '1.0.0';
    this.instructions = options.instructions || 'Analyze 代码知识图谱工具集：建图、查引用、看依赖、搜符号、追踪调用链、影响面分析';
  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.httpServer.listen(this.port, this.host, () => {
        ...
        console.log('==============================================');
        console.log('  Analyze MCP Server (代码知识图谱)');
        console.log('----------------------------------------------');
