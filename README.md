...
| [`cryptox/`](./cryptox/) | Cryptox | 8353 | 9 | 哈希 / 编码 / 加密（摘要 / HMAC / 文件校验和 / 编解码 / JWT 解析验签 / UUID 与短 ID / 密码生成与强度评估 / AES-256-GCM 加解密，全程本地无网络） |

下一个新工具集用 **8354**。测试端口约定：`18000 + 正式端口`（如 8348 → 18348、8353 → 18353）。
...
| cryptox | `npm test`（159 项） | `node test/mcp.e2e.js`（66 项） |

合计 **单测 1488 项 + 端到端 598 项**，全部通过、无跳过。
