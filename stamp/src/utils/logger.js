...
/**
 * Stamp 简易文件日志
 * 日志写到 <工具集根>/logs/stamp.log，每次追加，带时间戳
 */
const fs = require('fs');
const path = require('path');

const LOG_DIR = path.join(__dirname, '..', '..', 'logs');
const LOG_FILE = path.join(LOG_DIR, 'stamp.log');
...
function write(level, msg) {
  try {
    ...
    if (level === 'ERROR') console.error('[stamp]', line.trim());
    else console.log('[stamp]', line.trim());
  } catch (e) {
