...
class McpStreamableHttpServer {
  constructor(options = {}) {
    ...
    this.serverName = options.serverName || 'jsonx-mcp';
    this.serverVersion = options.serverVersion || '1.0.0';
    this.instructions = options.instructions || 'Jsonx 数据格式工具集：JSON/YAML/CSV/TSV 互转 / JSON 路径查询 / 结构化 diff / schema 推断 / 聚合统计';
  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.httpServer.listen(this.port, this.host, () => {
        ...
        console.log('==============================================');
        console.log('  Jsonx MCP Server (数据格式处理)');
        console.log('----------------------------------------------');
