...
class McpStreamableHttpServer {
  constructor(options = {}) {
    ...
    this.serverName = options.serverName || 'search-mcp';
    this.serverVersion = options.serverVersion || '1.0.0';
    this.instructions = options.instructions || 'Search 聚合搜索工具集：必应/百度/DuckDuckGo 搜索（折叠摘要+翻页）、结果全文展开、搜索词联想';
  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.httpServer.listen(this.port, this.host, () => {
        ...
        console.log('==============================================');
        console.log('  Search MCP Server (聚合搜索)');
        console.log('----------------------------------------------');
