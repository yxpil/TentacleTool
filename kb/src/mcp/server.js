...
class McpStreamableHttpServer {
  constructor(options = {}) {
    ...
    this.serverName = options.serverName || 'kb-mcp';
    this.serverVersion = options.serverVersion || '1.0.0';
    this.instructions = options.instructions || 'KB 知识库工具集：把多个 MySQL 表当作知识库来查（列出数据源 / 看表结构 / 跨表搜索 / 只读 SQL / 概览统计）';
  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.httpServer.listen(this.port, this.host, () => {
        ...
        console.log('==============================================');
        console.log('  KB MCP Server (MySQL 知识库)');
        console.log('----------------------------------------------');
