...
class McpStreamableHttpServer {
  constructor(options = {}) {
    ...
    this.serverName = options.serverName || 'cryptox-mcp';
    this.serverVersion = options.serverVersion || '1.0.0';
    this.instructions = options.instructions || 'Cryptox 哈希与编码工具集：哈希 / HMAC / 编解码 / JWT 解析 / UUID 与密码生成 / 文件校验和 / AES-256-GCM 加解密';
  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.httpServer.listen(this.port, this.host, () => {
        ...
        console.log('==============================================');
        console.log('  Cryptox MCP Server (哈希 / 编码 / 加密)');
        console.log('----------------------------------------------');
