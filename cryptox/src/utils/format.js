'use strict';
/**
 * 输出格式化：cryptox 各工具共用的紧凑文本 / 表格 / 截断逻辑
 *
 * 与 jsonx / kb / calc 等工具集保持同一套约定：
 *  - 结果以 Markdown 表格或紧凑列表给出，一行一条
 *  - 单元格与总输出双重截断，宁可提示"已截断"也不要把上下文撑爆
 *  - 顶部给结论，底部给"下一步能做什么"
 *  - 列宽按显示宽度算（CJK 计 2 列），null 显示为 —
 */
...
function size(n) {
  ...
  return (i === 0 ? String(n) : v.toFixed(v >= 10 || i === 0 ? 0 : 1)) + ' ' + u[i];
}
...
module.exports = {
  cell, table, kv, list, ms, size, displayWidth, pad, clip, indentBlock
};
