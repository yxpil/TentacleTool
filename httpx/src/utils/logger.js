...
/**
 * Httpx 简易文件日志
 * 日志写到 <项目根>/logs/httpx.log，每次追加，带时间戳
 */
const fs = require('fs');
const path = require('path');

const LOG_DIR = path.join(__dirname, '..', '..', 'logs');
const LOG_FILE = path.join(LOG_DIR, 'httpx.log');
...
function write(level, msg) {
  try {
    ...
    if (level === 'ERROR') console.error('[httpx]', line.trim());
    else console.log('[httpx]', line.trim());
  } catch (e) {
